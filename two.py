...
def p(x, th):
    c1 = 0.7 * norm(th[0], 2).pdf(x)
    c2 = 0.3 * norm(th[1], 2).pdf(x)
    return c1 + c2
...
def q(z, x, th):
    if z == 0: return 0.7 * norm(th[0], 2).pdf(x) / p(x, th)
    elif z == 1: return 0.3 * norm(th[1], 2).pdf(x) / p(x, th)
...
# Map log_likelihoods
with open('two/logs.txt', 'r+') as lf:
    if RW['log'] == 'w':
...
for t in range(11):
    thetas.append([])
    for z in range(2):
        q_l = [q(z, x, thetas[-2]) for x in dataset] # E-step
        numer = sum([q_l[i]*dataset[i] for i in range(len(dataset))])
        denom = sum(q_l)
        thetas[-1].append(numer/denom) # M-step
...
Qs = []
with open('two/aux0.txt', 'r+') as qf0:
    if RW['q0'] == 'w':
...
Qs = []
with open('two/aux1.txt', 'r+') as qf1:
    if RW['q1'] == 'w':
...
for i in range(len(Qs)):
    plt.plot(thetas[i][0], thetas[i][1], '*', color=cmap(nmap(Qs[i])), markersize=10)
...
for x in dataset:
    ...
    for z in range(2):
        alpha /= 1.1
        posterior = q(z, x, thetas[-2])
        s_vals[z].append(s_vals[z][-1] + alpha*(np.array([posterior, posterior*x]) - s_vals[z][-1]))
        thetas[-1][z] = s_vals[z][-1][1]/s_vals[z][-1][0]

plt.title(r'Online EM algorithm over Dataset $\mathbb{D}$ with Model M2')
plt.plot([th[0] for th in thetas], [th[1] for th in thetas], '*b-', color='black')
plt.show()
